...
class TestMeals(GroundTests):
    ...
    def test_empty_inputs_in_put(self):
    	...
    	response = self.tester.put('/api/v1/meals/'+str(self.test_meal.meal_id), data=json.dumps(self.mod_emp_meal), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertEqual(data['message'], 'Please enter all the details')
    ...
    def test_meal_name_not_string(self):
    	...
    	response = self.tester.put('/api/v1/meals/'+str(self.test_meal.meal_id), data=json.dumps(self.mod_int_meal), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertEqual(data['message'], 'Please enter a string value for meal')
    ...
    def test_modified_success(self):
    	...
    	response = self.tester.put('/api/v1/meals/'+str(self.test_meal.meal_id), data=json.dumps(self.modify_meal), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
    ...
    def test_delete_when_meal_unavailable(self):
    	...
    	response = self.tester.delete('/api/v1/meals/24567876543234567876543', content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertEqual(data['message'], "The meal was not found")
    ...
    def test_delete_success(self):
    	...
    	response = self.tester.delete('/api/v1/meals/'+str(self.test_meal.meal_id), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
